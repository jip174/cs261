 ...
 * Email:phillij6@oregonstate.edu
 */
...
void listQueueInit(struct listQueue *q) {
	...
	struct link *qLink = (struct link*) malloc(sizeof(struct link));
	assert(qLink != 0);
	qLink->next = 0;
	q->firstLink = q->lastLink = qLink;
...
struct listQueue * listQueueCreate()
{
	...
	//newQueue->capacity = 0;
	//newQueue->firstLink = newQueue->lastLink = '\0';
...
{
	// FIXME: you must write this
	//struct link *newStack = (struct link*)malloc(sizeof(struct link));
	//assert(newStack != 0);
...
void linkedListStackPush(struct linkedListStack *s, TYPE d) {
	...
	s->Q1->firstLink = newList;
	printf("value: %d\n", s->Q1->firstLink->value);
	if (s->Q1->firstLink->next != 0)
	{
		printf("value: %d\n", s->Q1->firstLink->next->value);
	}
		s->structSize++;*/
...
void linkedListStackPop(struct linkedListStack *s) {
	...
	if (!linkedListStackIsEmpty(s))
	{
		...
		if (s->Q1->firstLink != s->Q1->lastLink)
		{
