...
{
	// FIXME: you must write this
	struct Link *listValue;
	assert(linkedListIsEmpty(list));
	listValue = list->frontSentinel->next;

	while (listValue != list->backSentinel)
	{
		if (EQ(listValue->value, value))
		{
			return 1;
